...
/* ================= STATE ================= */
const state = { words: [], quiz: null, view: 'catch', user: null };
...
function initAuthUI(){
  document.querySelectorAll('.auth-tab-btn').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      ...
      document.getElementById('authSubmit').textContent = authMode==='signin' ? 'เข้าสู่ระบบ' : 'สมัครสมาชิก';
...
async function handleAuthSubmit(){
  ...
  if(!email || !password){
    msg.textContent = 'กรอกอีเมลและรหัสผ่านให้ครบ';
  ...
  if(authMode === 'signin'){
    ...
    if(error){ msg.textContent = error.message; msg.classList.add('error'); return; }
    msg.textContent = 'สมัครสำเร็จ — ถ้าโปรเจกต์เปิดใช้ยืนยันอีเมล ให้เช็คอินบ็อกซ์ก่อนเข้าสู่ระบบ';
...
async function loadWords(){
  const {data, error} = await sb.from('words').select('*').order('added_at', {ascending:false});
...
async function updateWordRow(id, patch){
  ...
  const {error} = await sb.from('words').update(dbPatch).eq('id', id);
...
async function loadQuizRow(dateKey){
  const {data, error} = await sb.from('daily_quizzes').select('*').eq('quiz_date', dateKey).maybeSingle();
...
async function fetchWordData(word){
  ...
  try{
    const res2 = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(word)}&langpair=en|th`);
    if(res2.ok){
      const json2 = await res2.json();
      if(json2 && json2.responseData && json2.responseData.translatedText){
        meaningTh = json2.responseData.translatedText;
      }
    }
  }catch(e){ /* ignore */ }
